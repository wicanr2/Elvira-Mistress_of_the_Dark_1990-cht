...
這個專案的三顆崩潰類問題,來源分別是:第一顆從玩家的 macOS crash report 定位(出事位址正好是緩衝區結束 +1);第二顆是修完第一顆後 ASan 掃出來的;第三顆(字型檔損毀時不擋)是讀程式碼時發現的。**沒有一顆是「玩一玩發現」的**——修正前後的 ASan 對照(1 顆 → 0 顆)則是三顆共同的驗收依據。
