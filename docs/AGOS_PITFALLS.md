...
`getStringPtrByID()` 看似只是取畫面文字，但 AGOS 也會把回傳值用於腳本條件比較與
內部存檔名稱。若在這個共用出口無條件套用譯表，翻譯就會改變遊戲狀態：例如英文
`The soldier` 與 `the soldier` 原本不分大小寫相等，中文「那名士兵」與「士兵」
卻不相等，敵人分類失敗後會把 0 當成圖片編號並誤載 `002.VGA`。

正解是把顯示與語意分流。顯示呼叫維持預設翻譯；條件比較、資源名稱、存檔名稱、
腳本鍵值與其他識別用途必須明確取得原始資料。不可藉由統一眼前幾條譯文修好測例，
因為那會把引擎契約藏進翻譯內容，換一條字串或另一種語言就可能再壞。

驗證至少要包含：同一路徑的繁中／英文差分、原始字串與譯文的等價性對照、語意
呼叫點盤點，以及真正走到受影響腳本分支的遊玩回歸。只驗「畫面顯示正確」不足以
證明翻譯掛勾安全。
