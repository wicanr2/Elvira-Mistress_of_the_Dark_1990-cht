...
> **v1.2.5**：把翻譯限制在顯示用途。戰鬥腳本的敵人名稱比較與內部存檔名稱
> 一律讀取原始英文識別字，修正繁中模式進入部分守衛戰時誤載 `002.VGA` 的問題。
...
| [`docs/BUG_ZONE_BOUNDS.md`](docs/BUG_ZONE_BOUNDS.md) | **隨機當機追查報告**：zone 上界失守導致物件內部越界讀——不會當場炸、每次當的位置都不同、而且 ASan 抓不到的那一類 |
