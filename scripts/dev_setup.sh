#!/bin/bash
# dev-setup 接續包:從零把《古堡禁地》(Elvira 1) 繁中化開發環境架起來。
...
if [ "${1:-}" != "--rebuild-only" ]; then
  step "1/5 建置 docker 映像 agos-build"
  docker build -t agos-build -f docker/Dockerfile docker/
...
mkdir -p run_game
for f in elvira1_zh16.dcjk elvira1_zh24.dcjk elvira1_zh.tab; do
  [ -f "run_game/$f" ] || cp "fonts/$f" "run_game/$f" 2>/dev/null || \
    cp "translations/$f" "run_game/$f" 2>/dev/null || true
done
if [ ! -f run_game/gamepc ] && [ ! -f run_game/GAMEPC ]; then
  cat <<'MSG'

  ⚠ run_game/ 尚無遊戲原檔(gamepc、tables01…、start、*.vga 等)。
    請把你合法擁有的 Elvira - Mistress of the Dark(Floppy/DOS) 全部檔案複製進 run_game/ 後即可執行:
...
echo
echo "完成。開發指南見 docs/DEV_SETUP.md。"
